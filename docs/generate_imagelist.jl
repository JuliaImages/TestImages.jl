function generate_imagelist(root, (testimage, database), out_file, preface)
    ...
    for i in 1:N
        if length(sizes[i]) == 2
            ...
            thumbnail = clamp01nan!(imresize(imgs[i], HEIGHT, width))
        ...
        elseif length(sizes[i]) == 3
            ...
            width = round(Int, HEIGHT / sizes[i][1] * sizes[i][2])
            img = collect(imgs[i])
            thumbnail = Array{floattype(eltype(img)),3}(undef,HEIGHT,width,sizes[i][3])
            for j in 1:sizes[i][3]
                thumbnail[:,:,j] = clamp01nan!(imresize(img[:,:,j], HEIGHT, width))
            end
        ...
        elseif length(sizes[i]) == 4
            ...
            thumbnail = Array{floattype(eltype(img)),3}(undef,HEIGHT,width,n_frames)
            for j in 1:n_frames
                thumbnail[:,:,j] = clamp01nan!(imresize(img[:,:,j], HEIGHT, width))
            end
            path = joinpath(root, "thumbnails", names[i]*".gif")
